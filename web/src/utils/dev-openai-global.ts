import type { OpenAiGlobals, TodoTask } from "./types";
...
export function setupDevOpenAiGlobal(): void {
  ...
  (window as any).openai = {
    ...
    callTool: async (name: string, args: Record<string, unknown>) => {
      ...
      let message = "Unsupported tool";
      if (name === "todo-get-tasks") {
        message = JSON.stringify({ tasks: mockTasks });
      } else if (name === "todo-add-task") {
        ...
        if (title) {
          ...
          mockTasks.push(task);
          message = JSON.stringify({ tasks: mockTasks, added: task });
        } else {
          message = JSON.stringify({ error: "Missing title" });
        }
      } else if (name === "todo-remove-task") {
        const id = String(args.id ?? "").trim();
        const index = mockTasks.findIndex((task) => task.id === id);
        if (index >= 0) {
          const removed = mockTasks.splice(index, 1)[0];
          message = JSON.stringify({ tasks: mockTasks, removed });
        } else {
          message = JSON.stringify({ error: "Task not found" });
        }
      }
      mockOpenAi.widgetState = { tasks: mockTasks };
